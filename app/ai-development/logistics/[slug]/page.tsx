// app/ai-development/logistics/[slug]/page.tsx
...
// Generate static paths for all Logistics AI city pages
export async function generateStaticParams() {
  const slugs = getAllLogisticsAICityPageSlugs();
  // Map "logistics-atlanta" -> "atlanta"
  const citySlugs = slugs.filter((s) => s !== 'logistics').map((s) => s.replace('logistics-', ''));
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Page data keys are prefixed with "logistics-", map the URL slug back to the data key
  const dataKey = `logistics-${slug}`;
  const pageData = getLogisticsAICityPageDataBySlug(dataKey);
...
// Main page component
export default async function LogisticsAICityPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  // Map the URL slug (e.g. "atlanta") to the data key (e.g. "logistics-atlanta")
  const dataKey = `logistics-${slug}`;
  const pageData = getLogisticsAICityPageDataBySlug(dataKey);
  ...
  const cityName = pageData.h1.split('  ')[0].replace('Logistics AI Development in ', '');
  ...
  return (
    <PageLayout withGrain>
      ...
      <StatsSection
        title={`Logistics AI in ${cityName}: Key Metrics`}
        stats={pageData.overview.stats ?? []}
      />
