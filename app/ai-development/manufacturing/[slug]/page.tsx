// app/ai-development/manufacturing/[slug]/page.tsx
...
// Generate static paths for all Manufacturing AI city pages
export async function generateStaticParams() {
  const slugs = getAllManufacturingAICityPageSlugs();
  // Map "manufacturing-atlanta" -> "atlanta"
  const citySlugs = slugs.filter((s) => s !== 'manufacturing').map((s) => s.replace('manufacturing-', ''));
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Page data keys are prefixed with "manufacturing-", map the URL slug back to the data key
  const dataKey = `manufacturing-${slug}`;
  const pageData = getManufacturingAICityPageDataBySlug(dataKey);
...
// Main page component
export default async function ManufacturingAICityPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  // Map the URL slug (e.g. "atlanta") to the data key (e.g. "manufacturing-atlanta")
  const dataKey = `manufacturing-${slug}`;
  const pageData = getManufacturingAICityPageDataBySlug(dataKey);
  ...
  const cityName = pageData.h1.split('  ')[0].replace('Manufacturing AI Development in ', '');
  ...
  return (
    <PageLayout withGrain>
      ...
      <StatsSection
        title={`Manufacturing AI in ${cityName}: Key Metrics`}
        stats={pageData.overview.stats ?? []}
      />
