// app/ai-development/edtech/[slug]/page.tsx
...
// Generate static paths for all EdTech AI city pages
export async function generateStaticParams() {
  const slugs = getAllEdTechAICityPageSlugs();
  // Map "edtech-boston" -> "boston"
  const citySlugs = slugs.map((s) => s.replace('edtech-', ''));
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Page data keys are prefixed with "edtech-", map the URL slug back to the data key
  const dataKey = `edtech-${slug}`;
  const pageData = getEdTechAICityPageDataBySlug(dataKey);
...
// Main page component
export default async function EdTechAICityPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  // Map the URL slug (e.g. "boston") to the data key (e.g. "edtech-boston")
  const dataKey = `edtech-${slug}`;
  const pageData = getEdTechAICityPageDataBySlug(dataKey);
  ...
  const cityName = pageData.h1.split('  ')[0].replace('EdTech AI Development in ', '');
  ...
  return (
    <PageLayout withGrain>
      ...
      <StatsSection
        title={`EdTech AI in ${cityName}: Key Metrics`}
        stats={pageData.overview.stats ?? []}
      />
