// app/ai-development/insurance/[slug]/page.tsx
...
// Generate static paths for all Insurance AI city pages
export async function generateStaticParams() {
  const slugs = getAllInsuranceAICityPageSlugs();
  // Map "insurance-atlanta" -> "atlanta"
  const citySlugs = slugs.filter((s) => s !== 'insurance').map((s) => s.replace('insurance-', ''));
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Page data keys are prefixed with "insurance-", map the URL slug back to the data key
  const dataKey = `insurance-${slug}`;
  const pageData = getInsuranceAICityPageDataBySlug(dataKey);
...
// Main page component
export default async function InsuranceAICityPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  // Map the URL slug (e.g. "atlanta") to the data key (e.g. "insurance-atlanta")
  const dataKey = `insurance-${slug}`;
  const pageData = getInsuranceAICityPageDataBySlug(dataKey);
  ...
  const cityName = pageData.h1.split('  ')[0].replace('Insurance AI Development in ', '');
  ...
  return (
    <PageLayout withGrain>
      ...
      <StatsSection
        title={`Insurance AI in ${cityName}: Key Metrics`}
        stats={pageData.overview.stats ?? []}
      />
