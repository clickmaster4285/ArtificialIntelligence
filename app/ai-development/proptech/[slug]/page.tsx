// app/ai-development/proptech/[slug]/page.tsx
...
import {
  getAllPropTechAICityPageSlugs,
  getPropTechAICityPageDataBySlug,
} from '@/data/proptech-ai-city-pages-data';

// Generate static paths for all PropTech AI city pages
export async function generateStaticParams() {
  const slugs = getAllPropTechAICityPageSlugs();
  // Map "proptech-houston" -> "houston"
  const citySlugs = slugs.map((s) => s.replace('proptech-', ''));
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Page data keys are prefixed with "proptech-", map the URL slug back to the data key
  const dataKey = `proptech-${slug}`;
  const pageData = getPropTechAICityPageDataBySlug(dataKey);
...
// Main page component
export default async function PropTechAICityPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  // Map the URL slug (e.g. "houston") to the data key (e.g. "proptech-houston")
  const dataKey = `proptech-${slug}`;
  const pageData = getPropTechAICityPageDataBySlug(dataKey);
  ...
  const cityName = pageData.h1.split('  ')[0].replace('PropTech AI Development in ', '');
  ...
  return (
    <PageLayout withGrain>
      ...
      <StatsSection
        title={`PropTech AI in ${cityName}: Key Metrics`}
        stats={pageData.overview.stats ?? []}
      />
