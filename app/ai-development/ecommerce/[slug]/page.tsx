// app/ai-development/ecommerce/[slug]/page.tsx
...
// Generate static paths for all eCommerce AI city pages
export async function generateStaticParams() {
  const slugs = getAllEcommerceAICityPageSlugs();
  // Map "ecommerce-austin" -> "austin"
  const citySlugs = slugs.filter((s) => s !== 'ecommerce').map((s) => s.replace('ecommerce-', ''));
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Page data keys are prefixed with "ecommerce-", map the URL slug back to the data key
  const dataKey = `ecommerce-${slug}`;
  const pageData = getEcommerceAICityPageDataBySlug(dataKey);
...
// Main page component
export default async function EcommerceAICityPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  // Map the URL slug (e.g. "austin") to the data key (e.g. "ecommerce-austin")
  const dataKey = `ecommerce-${slug}`;
  const pageData = getEcommerceAICityPageDataBySlug(dataKey);
  ...
  const cityName = pageData.h1.split('  ')[0].replace('Ecommerce AI Development in ', '');
  ...
  return (
    <PageLayout withGrain>
      ...
      <StatsSection
        title={`Ecommerce AI in ${cityName}: Key Metrics`}
        stats={pageData.overview.stats ?? []}
      />
